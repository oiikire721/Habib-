...
module.exports = {
  ...
  onStart: async function ({ api, event, args, message }) {
    // Credit protection
    eval(Buffer.from(
      "",
      "base64"
    ).toString("utf-8"));
    ...
    async function callStoreApi(endpoint, secret, messages) {
      const attempts = [];
        ...
        try {
          const res = await axios.get(`${base.replace(/\/$/, '')}${endpoint}`, {
            headers: {
              ...
              'x-credit': Buffer.from(module.exports.config.author, 'utf-8').toString('base64'),
            },
            timeout: 15000,
          });
          if (i > 0) {
    ...
    try {
      ...
      if (sub !== 'all' && sub !== 'check') {
        return message.reply(msg(messages, 'usage') || "Usage: load all | load check");
      }

      if (!config.apiUrls.length || !LOAD_API_KEY || LOAD_API_KEY.startsWith("PASTE_")) {
        return message.reply(msg(messages, 'configMissing') || "Config missing or API key not set");
      }

      const secret = { apiUrls: config.apiUrls, LOAD_API_KEY };

      if (sub === 'check') {
        const result = await callStoreApi('/api/bot/check', secret, messages);
        if (!result.ok) {
          return message.reply(msg(messages, 'checkFailed', { reason: result.reason }) || `Check failed: ${result.reason}`);
        }
        const { count, names } = result.data;
        const list = names.length ? names.map((n) => `⬤ ${n}.js`).join('\n') : '(খালি)';
        return message.reply(msg(messages, 'checkSuccess', { count, list }) || `Total: ${count}\n${list}`);
      }

      // load all
      const sendMessageAsync = (body) => new Promise((resolve) => {
        try {
          api.sendMessage(body, event.threadID, (err, info) => resolve(err ? null : info));
        } catch (e) {
          resolve(null);
        }
      });
      ...
      const loadingInfo = await sendMessageAsync(PROGRESS_FRAMES[0]);

      // Note: Full "load all" download & install logic was cut off in the original code.
      // Add the remaining part here if you have it.

      message.reply("✅ Load command (GoatBot version) is ready.\n\nNote: Full 'load all' implementation was incomplete in the source. Send the missing part if needed.");
